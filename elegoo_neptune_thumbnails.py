...
class SliceData:
    ...
    def __init__(self, time_seconds: int, printer_model: str, model_height: float, layer_count: int ,filament_grams: float,
                 filament_cost: float,filament_length: str = None, layer_height: float = -1, currency: str = None):
        self.printer_model: str = printer_model
        self.time_seconds: int = time_seconds
        self.model_height: float = model_height
        self.layer_count: float = layer_count
        self.filament_grams: float = filament_grams
        self.filament_cost: float = filament_cost
        self.layer_height: float = layer_height
        self.filament_length: float = filament_length
...
class ElegooNeptuneThumbnails:
    ...
    def _get_slice_data(self) -> SliceData:
        ...
        attribute_mapping: dict[str, str] = {
            ...
            " filament used [mm] = ": "filament_length",
        ...
        filament_length: str = attributes.get("filament_length", None)
        if filament_length is not None:
            total_length: float = float(attributes.get("filament_length", 0))/1000
            filament_length = str(total_length) + "m"
        ...
        return SliceData(
            ...
            layer_count=int(attributes.get("layer_count", "-1")),
    ...
    def _add_CURA_COMPATIBLE_HEADER_BLOCK(self) -> str:
        """
        Adds a Cura compatible header block to the gcode file
        """
        gcode_header: str = "\r\n;CURA_COMPATIBLE_HEADER_BLOCK_START\n"
        gcode_header += ";TIME:" + self._slice_data.time_seconds.__str__() + "\n"
        gcode_header += ";Filament used: " + self._slice_data.filament_length.__str__() + "\n"
        gcode_header += ";Filament weight = ." + self._slice_data.filament_grams.__str__() + ".\n" 
        gcode_header += ";Layer height: " + self._slice_data.layer_height.__str__() + "\n"
      #  gcode_header += ";MINZ: 0.20\n"
        gcode_header += ";MAXZ:" + self._slice_data.model_height.__str__() + "\n"
        gcode_header += ";LAYER_COUNT:" + self._slice_data.layer_count.__str__() + "\n"
        gcode_header += ";CURA_COMPATIBLE_HEADER_BLOCK_END\r\n"
        return gcode_header
    ...
    def add_thumbnail_prefix(self) -> None:
        ...
        if ';gimage:' not in g_code and ';simage:' not in g_code:
            ...
            gcode_prefix += self._add_CURA_COMPATIBLE_HEADER_BLOCK()
